...
export async function listRecords(owner: string) {
  ...
  >`SELECT document FROM dodam_records WHERE owner_id = ${owner} ORDER BY document->>'date' DESC, updated_at DESC`;
...
export async function claimImage(owner: string, limit: number) {
  const sql = await ready();
  const rows =
    await sql`INSERT INTO dodam_image_usage (owner_id, day, count) VALUES (${owner}, (NOW() AT TIME ZONE 'Asia/Seoul')::date, 1) ON CONFLICT (owner_id, day) DO UPDATE SET count = dodam_image_usage.count + 1 WHERE dodam_image_usage.count < ${limit} RETURNING count`;
  return rows.length > 0;
}
export async function getProfile(
  owner: string,
): Promise<{ institution: string; classRole: string } | null> {
  const sql = await ready();
  const rows =
    await sql`SELECT institution, class_role FROM dodam_profiles WHERE owner_id = ${owner}`;
  return rows[0]
    ? { institution: rows[0].institution, classRole: rows[0].class_role }
    : null;
}
export async function saveProfile(
  owner: string,
  profile: { institution: string; classRole: string },
) {
  const sql = await ready();
